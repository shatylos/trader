package buyCheapSellHigh

import (
	"errors"
	"github.com/shatylos/trader/internal/domain"
	"github.com/shatylos/trader/tools"
	"github.com/shatylos/trader/tools/logger"
	"github.com/shatylos/trader/tools/type"
)

func (s *BuyCheapSellHigh) SetConfig(strategyConfig interface{}, domainConfig map[string]interface{}) error {

	var err error

	configMap, ok := strategyConfig.(map[interface{}]interface{})
	if !ok {
		return tools.AppError{
			Message: "Config for the strategy buyCheapSellHigh is not valid",
		}
	}

	s.Id, err = _type.ToString(configMap["id"])
	if err != nil {
		return tools.AppError{
			Message:     "The field id is empty or contains not correct value type. Expects string value",
			ParentError: err,
		}
	}

	s.CoinPare, err = _type.ToString(configMap["coin_pare"])
	if err != nil {
		return tools.AppError{
			Message:     "The field coin_pare is empty or contains not correct value type",
			ParentError: err,
		}
	}

	var enabled int64
	enabled, err = _type.ToInt64(configMap["enabled"])
	if err != nil {
		logger.Error("The field enabled is empty or contains not correct value type. Expects 1 or 0")
		err = tools.AppError{
			Message:     "The field enabled is empty or contains not correct value type. Expects 1 or 0",
			ParentError: err,
		}
		return err
	}
	if enabled == 1 {
		s.Enabled = true
	} else {
		s.Enabled = false
	}

	s.MainCurrency, err = _type.ToString(configMap["main_currency"])
	if err != nil {
		return tools.AppError{
			Message:     "The field main_currency is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.TradeCurrency, err = _type.ToString(configMap["trade_currency"])
	if err != nil {
		return tools.AppError{
			Message:     "The field trade_currency is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.TimeoutSeconds, err = _type.ToTimeDuration(configMap["timeout_seconds"])
	if err != nil {
		return tools.AppError{
			Message:     "The field timeout_seconds is empty or contains not correct value",
			ParentError: err,
		}
	}

	s.Resolution, err = _type.ToString(configMap["resolution"])
	if err != nil {
		return tools.AppError{
			Message:     "The field resolution is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.LongTermMaxPrice, err = _type.ToFloat64(configMap["long_term_max_price"])
	if err != nil {
		return tools.AppError{
			Message:     "The field long_term_max_price is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.LongTermMinPrice, err = _type.ToFloat64(configMap["long_term_min_price"])
	if err != nil {
		return tools.AppError{
			Message:     "The field long_term_min_price is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.LongTermPercentBuffer, err = _type.ToFloat64(configMap["long_term_percent_buffer"])
	if err != nil {
		return tools.AppError{
			Message:     "The field long_term_percent_buffer is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.MaxQtyDiff, err = _type.ToFloat64(configMap["max_qty_diff"])
	if err != nil {
		return tools.AppError{
			Message:     "The field max_qty_diff is empty or contains not correct value type. Expects float64 value",
			ParentError: err,
		}
	}

	s.MinQty, err = _type.ToFloat64(configMap["min_qty"])
	if err != nil {
		return tools.AppError{
			Message:     "The field min_qty is empty or contains not correct value type. Expects float64 value",
			ParentError: err,
		}
	}

	s.MainCurrencyPrecision, err = _type.ToInt64(configMap["main_currency_precision"])
	if err != nil {
		return tools.AppError{
			Message:     "The field main_currency_precision is empty or contains not correct value type. Expects int64 value",
			ParentError: err,
		}
	}

	s.PurchaseVolumePrecision, err = _type.ToInt64(configMap["purchase_volume_precision"])
	if err != nil {
		return tools.AppError{
			Message:     "The field purchase_volume_precision is empty or contains not correct value type. Expects int64 value",
			ParentError: err,
		}
	}

	s.PurchasePricePrecision, err = _type.ToInt64(configMap["purchase_price_precision"])
	if err != nil {
		return tools.AppError{
			Message:     "The field purchase_price_precision is empty or contains not correct value type. Expects int64 value",
			ParentError: err,
		}
	}

	s.CommissionPercent, err = _type.ToFloat64(configMap["commission_percent"])
	if err != nil {
		return tools.AppError{
			Message:     "The field commission_percent is empty or contains not correct value type. Expects float64 value",
			ParentError: err,
		}
	}

	s.MinutesToReducePriceRange, err = _type.ToInt64(configMap["minutes_to_reduce_price_range"])
	if err != nil {
		return tools.AppError{
			Message:     "The field minutes_to_reduce_price_range is empty or contains not correct value type. Expects int64 value",
			ParentError: err,
		}
	}

	s.AvgPriceCandleLimit, err = _type.ToInt64(configMap["avg_price_candle_limit"])
	if err != nil || s.AvgPriceCandleLimit < 1 {
		return tools.AppError{
			Message:     "The field avg_price_candle_limit is empty or contains not correct value type. Expects int64 value more than 1",
			ParentError: err,
		}
	}

	s.AvgPriceCandleOffset, err = _type.ToInt64(configMap["avg_price_candle_offset"])
	if err != nil || s.AvgPriceCandleOffset < 1 {
		return tools.AppError{
			Message:     "The field avg_price_candle_offset is empty or contains not correct value type. Expects int64 value more than 1",
			ParentError: err,
		}
	}

	s.ManualBuyPriceBeforeStart, err = _type.ToFloat64(configMap["manual_buy_price_before_start"])
	if err != nil && !errors.Is(err, tools.EmptyValueError) {
		return tools.AppError{
			Message:     "The field manual_buy_price_before_start contains not correct value type. Expects float64 value. Can be empty",
			ParentError: err,
		}
	}

	s.CostRanges, err = _type.ToInt64Slice(configMap["cost_ranges"])
	if err != nil {
		return tools.AppError{
			Message:     "The field cost_ranges is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.PercentRanges, err = _type.ToInt64Slice(configMap["percent_ranges"])
	if err != nil {
		return tools.AppError{
			Message:     "The field percent_ranges is empty or contains not correct value type",
			ParentError: err,
		}
	}

	s.WithdrawPercent, err = _type.ToFloat64(configMap["withdraw_percent"])
	if err != nil {
		return tools.AppError{
			Message:     "The field withdraw_percent is empty",
			ParentError: err,
		}
	}

	err = s.applyDomainConfig(configMap, domainConfig)
	if err != nil {
		return err
	}

	return nil
}

func (s *BuyCheapSellHigh) applyDomainConfig(configMap map[interface{}]interface{}, domainConfig map[string]interface{}) error {
	domainId, err := _type.ToString(configMap["domain"])
	if err != nil {
		return tools.AppError{
			Message:     "The field domain is empty or contains not correct value type",
			ParentError: err,
		}
	}
	domainConfigItem, ok := domainConfig[domainId].(map[interface{}]interface{})
	if !ok {
		return tools.AppError{
			Message: "The domain config is not valid. Domain value should be related to the domain config item",
		}
	}
	domainCode, err := _type.ToString(domainConfigItem["code"])
	if err != nil {
		return tools.AppError{
			Message:     "The field code is empty in domain config or contains not correct value type",
			ParentError: err,
		}
	}
	domainItem, err := domain.GetSpotDomain(domainCode)
	if err != nil {
		return err
	}

	err = domainItem.SetConfig(domainConfigItem)
	if err != nil {
		return err
	}
	s.Domain = domainItem

	return nil
}
